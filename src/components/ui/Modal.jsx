...
export function Modal({ open, title, children, onClose, width = 'max-w-lg' }) {
  if (!open) return null
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-tea-950/50 p-4 backdrop-blur-sm" onMouseDown={onClose}>
      <div className={`max-h-[92vh] w-full ${width} overflow-auto rounded-[22px] border border-white/60 bg-white shadow-2xl`} onMouseDown={(e) => e.stopPropagation()}>
