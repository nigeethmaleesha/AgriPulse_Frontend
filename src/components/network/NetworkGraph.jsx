...
export function NetworkGraph({ nodes = [], edges = [], edgeFlows = [], compact = false }) {
  ...
  if (!nodes.length) return <div className="flex min-h-[320px] items-center justify-center text-sm text-muted">No graph nodes available.</div>

  return (
    <div className={`grid-paper overflow-auto rounded-2xl border border-tea-950/10 bg-[#fbfbf7] ${compact ? 'min-h-[300px]' : 'min-h-[440px]'}`}>
      <svg viewBox="0 0 1000 500" className={`${compact ? 'min-w-[760px]' : 'min-w-[900px]'} w-full`} role="img" aria-label="Tea supply capacity network">
        <defs>
        ...
        {activeEdges.map((edge) => {
          ...
          const saturated = flow && flow.residualCapacityKgPerDay === 0
          const high = util >= 85
          const stroke = saturated ? '#C34E4E' : high ? '#C58A2C' : '#93a39a'
          ...
          return <g key={`${edge.id}-${edge.fromCode}-${edge.toCode}`}>
            <line x1={a.x + 48} y1={a.y} x2={b.x - 48} y2={b.y} stroke={stroke} strokeWidth={saturated ? 4 : 2.2} opacity=".9" markerEnd="url(#arrow)" />
        ...
        {Object.entries(positions).map(([code, pos]) => {
          ...
          return <g key={code}>
            <circle cx={pos.x} cy={pos.y} r="46" fill={nodeFill[pos.type] || '#2F6B4F'} opacity={node?.active ? 1 : .35} />
            ...
            <text x={pos.x} y={pos.y + 16} textAnchor="middle" fontSize="9" fontWeight="700" fill="rgba(255,255,255,.7)">{pos.type}</text>
          </g>
        })}
      </svg>
      <div className="flex flex-wrap gap-4 border-t border-tea-950/8 bg-white/65 px-4 py-3 text-xs text-muted">
        <span><b className="text-graphite">Edge label:</b> {edgeFlows?.length ? 'flow / capacity kg/day' : 'capacity kg/day'}</span>
        {edgeFlows?.length > 0 && <><span className="inline-flex items-center gap-1.5"><i className="h-2 w-5 rounded bg-critical" /> Saturated</span><span className="inline-flex items-center gap-1.5"><i className="h-2 w-5 rounded bg-amberui" /> High utilization</span></>}
      </div>
    </div>
  )
}
