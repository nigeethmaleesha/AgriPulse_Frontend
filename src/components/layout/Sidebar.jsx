...
import {
  Activity, Boxes, Calculator, ChevronLeft, ChevronRight, ClipboardList, Droplets, Gauge, GitBranch,
  LayoutDashboard, Leaf, ListOrdered, Network, ShieldAlert, Sprout, TestTube2, ThermometerSun, Truck
} from 'lucide-react'
    ...
    <aside className={`topographic fixed inset-y-0 left-0 z-40 hidden flex-col border-r border-white/8 text-white transition-all duration-200 lg:flex ${collapsed ? 'w-[78px]' : 'w-[264px]'}`}>
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        ...
        <div className="mt-5 rounded-2xl border border-white/10 bg-white/[.06] p-2">
          {!collapsed && <div className="mb-1 flex items-center justify-between px-2 py-1.5"><span className="text-xs font-bold text-white">Resource Allocation</span><span className="rounded bg-emerald-300/15 px-1.5 py-0.5 text-[9px] font-bold text-emerald-200">M02 · LIVE</span></div>}
      ...
      <div className="border-t border-white/10 p-3">
        {!collapsed && <div className="mb-3 rounded-xl border border-white/10 bg-black/10 p-3"><div className="flex items-center gap-2 text-xs font-semibold text-white/80"><span className="h-2 w-2 rounded-full bg-emerald-300" />All Modules Connected</div><div className="mt-1 text-[10px] leading-4 text-white/45">Resource Allocation (8083), Module 4 (8081), and Dispatch Engine (8082) connected.</div></div>}
